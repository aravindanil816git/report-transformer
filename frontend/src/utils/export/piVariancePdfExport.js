 ...
 */
export const exportPiVariancePdf = async ({
  data = [],
  meta = { brands: [] },
  config = {},
  mode = "warehouse",
  ...
  for (let gIdx = 0; gIdx < groupNames.length; gIdx++) {
    ...
    const cleanGroupName = groupName.replace(/^WH-/i, "").split(/\s+(?:RFL|FL)/i)[0].trim().toUpperCase();
    doc.setFontSize(22);
    doc.setFont("helvetica", "bold");
    ...
    renderPivotBlock(`${cleanGroupName} TOTAL`, `${items.length} shops`, items, true);

    // 2. Render Individual Shop Pivot Blocks
    items.forEach((shopItem) => {
      const shopTitle = shopItem.shop_name;
      renderPivotBlock(shopTitle, `${brands.length} brands`, [shopItem], false);
    });

    const cleanFileName = `pi_variance_purchase_instruction_${cleanGroupName.toLowerCase().replace(/[^a-z0-9]+/g, '_')}.pdf`;
    doc.save(cleanFileName);

    // Stagger downloads slightly (400ms delay) to prevent browser pop-up / download throttling
    if (gIdx < groupNames.length - 1) {
