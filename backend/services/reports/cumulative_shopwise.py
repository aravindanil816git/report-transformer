        ...
        **kwargs
        ...
        if (start_idx is None or end_idx is None) and start_date and end_date:
            try:
                ...
                print(f"[WARN] [cumulative_shopwise] Failed to parse start_date/end_date: {e}")
        ...
        if view == "cumulative":
            ...
            if start_date_param:
                ...
                res = supabase.table("reports").select("id, type, config, uploads")\
                    .in_("type", ["shop_sales_cumulative", "combined_shopwise"])\
                    .or_(f"config->>date1.like.{month_prefix}%,config->>start_date.like.{month_prefix}%,config->>date.like.{month_prefix}%")\
                    .execute()
